...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        if elastic_trans and len(elastic_trans) != 3:
            raise Exception('elastic transform requires three argument: (alpha, sigma, alpha_affine)')
    ...
    def random_transform(self, x, y):
        ...
        if self.elastic_trans:
            if np.random.random() < 0.5:
                x = np.rollaxis(x, img_channel_index, 3)
                y = np.rollaxis(y, img_channel_index, 3)
                im_merge = np.concatenate((x, y), axis=2)
                im_merge_t = elastic_transform(im_merge, im_merge.shape[1] * self.elastic_trans[0], \
                                               im_merge.shape[1] * self.elastic_trans[1], im_merge.shape[1] * self.elastic_trans[2])
                x = np.rollaxis(im_merge_t[:, :, 0:1], 2, img_channel_index)
                y = np.rollaxis(im_merge_t[:, :, 1:], 2, img_channel_index)
